...
Given the large number of varying parameters and resulting parameter combinations, not all results are reported in the main text for the sake of brevity. The results that support the claims in this section but nevertheless can't be reported here due to the limitation of space are reported in online supplementary materials (available at **(insert link here)**). 
...
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{1in}cccccccccccc}
\toprule
...
Sensitivity analyses were conducted on other combinations of rewiring probability ($p_r$), bounded confidence threshold ($\epsilon_{mean}$), convergence parameter ($\alpha$), and mean lack of confidence ($c_{mean}$), and no qualitative difference in the results is found (see online supplementary materials).


## Parameter Combinations Sustaining Pluralistic Ignorance at the Empirical Level
