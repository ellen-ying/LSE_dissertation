;; Model V2.0: new conceptualisation of norm beleif, new updating method
...
to-report binomial [ p n x ]  ;; give the pdf value of a binomial distribution
  let prob ( choose n x ) * ( p ^ x ) * ( ( 1 - p ) ^ ( n - x ) )
  report prob
end

to-report choose [ n x ]  ;; calculate combinatorics
...
to update-norm-belief
  ...
  let evi mean accessed-info  ;; the mean of (inferred) WWOH beliefs as eividence
  ...
  set valuation-norm-sd a * valuation-norm-sd ^ 2
end
