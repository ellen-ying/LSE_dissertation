;; Model V1.6: private belief normal and remain constant, norm belief normal, complex function for preference, with neighbour beliefs prossibily being accessible, small-world network type added
;; Bayesian updating function fixed
...
turtles-own [
  ...
  p-E-nH ;; conditional probability of the evidence given not H
  p-H-E ;; posterior belief
  q ;; ratio of posterior probability
]
...
to update-norm-belief
  ...
  if (p-H = 0) [set p-H 0.0001]
  if (p-H = 1) [set p-H 0.9999]
  set p-E-H binomial p-H n-neighbour x  ;; conditional probs of evidence
  set p-E-nH binomial (1 - p-H) n-neighbour x  ;; conditional probs of evidence
  let num (p-H * p-E-H)
  let denom ((1 - p-H) * p-E-nH)
  ifelse (denom = 0) [ ;; update based on Bayes' rule
    set p-H-E 1  ;; q converges to infinity and 1/(1+q) to 1
  ] [
    set q num / denom
    set p-H-E q / (1 + q)
  ]

  ;;if (n-neighbour < 2 * x + 1) [
  ;;  if (p-H = 1) [ set p-H-E 1]
  ;;  set q ( p-H / (1 - p-H) ) ^ ( 2 * x - n-neighbour + 1)
  ;;  set p-H-E q / (1 + q)
  ;;]
  ;;if (n-neighbour > 2 * x + 1) [
  ;;  if (p-H = 0) [ set p-H-E 1]
  ;;  set q ( p-H / (1 - p-H) ) ^ ( 2 * x - n-neighbour + 1)
  ;;  set p-H-E q / (1 + q)
  ;;]
  ;;if (n-neighbour = 2 * x + 1) [
  ;;  set p-H-E ( p-H / (1 - p-H) ) ^ ( 2 * x - n-neighbour + 1)
  ;;  set p-H-E q / (1 + q)
  ;;]
