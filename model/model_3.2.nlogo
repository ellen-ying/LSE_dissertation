...
turtles-own [
  ...
  ;; ntervention section
  credibility-perception ;; agents' perception of the credibility of the information
]
...
to set-turtle-characteristics
  ...
  ask turtles [
    ...
    set conf random-normal conf-mean conf-sd
    while [conf < 0] [
      set conf random-normal conf-mean conf-sd
    ]
  ]
...
to implement-intervention
  ;; summary info inttervention with source credibility perception - once
  if (intervention-type = "sum-info") [
    if (ticks = intervention-tick-1) [
...
676
192
mean and standard deviation of the normal distribution from with confiendence is drawn
9
0.0
1
